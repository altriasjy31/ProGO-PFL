...
class HANSampler(dgl.dataloading.Sampler):
    ...
    def sample(self, g, seeds, exclude_eids=None):  # exclude_eids is for compatibility with link prediction
        ...
        for ntype in input_nodes_dict:
            ntype_nodes = []
            meta_path_name_nodes_dict = input_nodes_dict[ntype]
            for meta_path_name in meta_path_name_nodes_dict:
                ntype_nodes.append(input_nodes_dict[ntype][meta_path_name])
            ntype_nodes = torch.cat(ntype_nodes, dim=0)
            input_nodes[ntype] = torch.unique(ntype_nodes)
        return input_nodes, seeds, ntype_mp_name_block_dict
